<template>
	<div class="audio-recorder-container">
		...
		<div class="audio-controls">
			...
			<button @click="auditionEffect" class="btn-audition">试听效果</button>
		</div>
		<div v-if="statusMessage" class="status-message">{{ statusMessage }}</div>
		<div v-if="effectAudioUrl" class="audio-preview">
			<h4>效果试听:</h4>
			<audio :src="effectAudioUrl" controls autoplay></audio>
		</div>
		<div v-if="audioPlayerUrl" class="audio-preview">
			<h4>录音预览:</h4>
			<audio :src="audioPlayerUrl" controls></audio>
			<div v-if="recognizedText" class="recognized-text" style="margin-top: 15px">
				<h4>识别结果:</h4>
				<p>{{ recognizedText }}</p>
			</div>
		</div>
...
const auditionEffect = async () => {
	statusMessage.value = '正在生成效果音频...'
	const payload = {
		uid: props.userId,
		token: props.token,
		text: '你好，这是一条语音克隆音频。|こんにちは、これは音声クローンオーディオです。|Hello, this is a voice clone audio。|你好，呢一条语音克隆音讯。|안녕하세요, 음성 클론 오디오입니다。',
	}
	...
	try {
		...
		if (response.ok && result.audio) {
			effectAudioUrl.value = `data:audio/wav;base64,${result.audio}`
			statusMessage.value = '效果音频已生成，可以播放。'
		} else {
			throw new Error(result.message || '生成效果音频失败')
		}
	} catch (error) {
		console.error('生成效果音频失败:', error)
		const errorMessage = error instanceof Error ? error.message : '未知错误'
		statusMessage.value = `生成失败: ${errorMessage}`
...
const uploadAudio = async () => {
	...
	const payload = {
		userid: props.userId,
		token: props.token,
		rec: audioBase64.value,
	}

	try {
		const response = await fetch('https://alumnispherepyapi.liy.ink/api/uploadvoice', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
		...
		// 检查返回的 code 是否为 200
		if (result.code === 200) {
			statusMessage.value = '上传成功并识别完成！'
			recognizedText.value = result.text
			//alert(`识别结果: ${result.text}`)
		} else {
			// 优先使用后端返回的错误信息
			throw new Error(result.message || `操作失败，代码: ${result.code}`)
		}
	} catch (error) {
