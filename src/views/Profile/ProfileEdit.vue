...
const longgitude = ref<number | null>(null);
...
const originalEducationInfo = ref({
	...
	longgitude: null as number | null,
});
...
const cancelEditingEducation = () => {
	...
	longgitude.value = originalEducationInfo.value.longgitude;
...
const saveEducationInfo = async () => {
	...
	if (universityChanged) {
		try {
			...
			if (validationResponse.ok && validationResult.code === 200) {
				...
				longgitude.value = validationResult.lng;
			} else {
				alert("大学名称无法识别，请填写详细的官方全称。");
	...
	if (university.value !== originalEducationInfo.value.university) {
		...
		changedData.longgitude = longgitude.value;
	}
	...
	try {
		...
		if (response.ok) {
			...
			if (data.status === 0 && data.data) {
				...
				longgitude.value = eduData.longgitude || null;
				...
				originalEducationInfo.value = {
					...
					longgitude: eduData.longgitude || null,
				};
			}
		}
	} catch (error) {
